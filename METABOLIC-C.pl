...
=head1 USAGE       
	...
	perl METABOLIC-C.pl -t 40 -in-gn Genome_files -kofam-db full -r omic_reads_parameters.txt -st illumina -gtdbtk-dir /path/to/gtdbtk/output -o METABOLIC_out	
		(use GTDB-Tk output files obtained with newer database versions, avoiding the pinned version of GTDB-Tk in METABOLIC-C. 
		To obtain the GTDB-Tk output, run the following command in a separate environment (installation instructions available at: https://ecogenomics.github.io/GTDBTk/installing/index.html) after changing the input parameters: 
		gtdbtk classify_wf --cpus numer_of_CPUS -x fasta --genome_dir path/to/input/dir --skip_ani_screen --out_dir path/to/output/dir)
...
=head1 OPTIONS
	...
	-gtdbtk-dir or -gtdb             [string]  The output folder of a GTDB-Tk "classify_wf" run performed beforehand. Its gtdbtk.bac120.summary.tsv and/or gtdbtk.ar53.summary.tsv are used and GTDB-Tk is not run. If this option is not given, GTDB-Tk is run as before; note that the pinned gtdbtk=2.3.2 supports GTDB releases up to R214, so a newer GTDB database requires this option.
...
# The GTDB-Tk pinned in the METABOLIC environment (gtdbtk=2.3.2) reads GTDB
# reference data up to R214. With a newer GTDB release, run GTDB-Tk separately
# and pass its output directory with -gtdbtk-dir; otherwise GTDB-Tk is run here
# as before.
...
if ($gtdbtk_dir){
	...
	system ("gtdbtk classify_wf --cpus $cpu_numbers -x fasta --genome_dir $input_genome_folder --skip_ani_screen --out_dir $gtdbtk_out 2> /dev/null");
}
